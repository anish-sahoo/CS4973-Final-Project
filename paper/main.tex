...
However, eye tracking has its own share of problems. The process of ensuring consistent results can be finicky and particular. First, it is hard to generalize eyes because humans have many different eye shapes and colors, and it is expected that an eye tracker should support all of these variations to the same level of accuracy. Furthermore, real-time computation efficiency is needed to make it viable for daily use. In order to tackle this problem, several subproblems need to be addressed, including eye-region detection, mapping eye to screen coordinates, and pupil localization under various lighting conditions, among others. Furthermore, it is generally known that laptop gaze trackers are not entirely accurate, but is most convenient for the purpose of this project.
...
\begin{enumerate}
    %\item{
    %https://arxiv.org/abs/1504.02863 - We use this dataset as it provides an excellent collection of gaze data. (expand on this, see https://www.mpi-inf.mpg.de/departments/computer-vision-and-machine-learning/research/gaze-based-human-computer-interaction/appearance-based-gaze-estimation-in-the-wild for more info on the dataset)
    %}
    ...
    \begin{itemize}
        ...
        \item This dataset provides an excellent collection of gaze data, which we have used in our project to train our model. 
    \end{itemize}
    }
    ...
    \begin{itemize}
        ...
        \item Our goal is to learn from the literature and create a model that takes the benefits of these approaches and can train and run efficiently on modern consumer devices.
    \end{itemize}
    }
\end{enumerate}
...
% TO DO 
For our dataset, we used MPIIGaze
...
% questions....
% how to make the most efficient? what architecture and implementation choices?
% ensure works on consumer-grade devices, involves testing and training on our personal laptops
...
Our dual-eye GazeNet architecture achieved a mean angular error of 1.44° on the validation set. The model demonstrates strong accuracy, with 96.81\% of predictions falling within 5° error and 99.21\% within 10° error.
...
Furthermore, we found success with running the calibration so that the eye tracker works as expected. However, there is still room for improvement. Although frames are collected over 2 seconds to reduce noise, some noise still remains. Not only that, but also the best results currently come from situations where the eyes are close to the screen during calibration, so there are limitations as to where the user can position themselves. Noise can be further reduced.  

\section{Conclusion}
v just some questions that i thought of to address, will figure out what to write after
\begin{enumerate}
    \item what did we discover? - 
    \item anything surprising? anything expected?
    \item what would we like to do in the future? anything we would change? if we had more time, what would we do?
\end{enumerate}

yada yada this is amazing for the future of humanity i gotchu >:)
